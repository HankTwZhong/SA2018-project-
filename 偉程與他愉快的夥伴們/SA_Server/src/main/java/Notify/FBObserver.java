...
public class FBObserver  implements IObserver {
    public void Update(Host host) {
        ...
        for(int i =0;i< host.getContactList().size();i++){
            ...
            ArrayList<String> addressForFaceBook = addressList.get("FB");
            for(int j=0;j<addressForFaceBook.size();j++) {
                System.out.println("send status change msg to " + host.getContactList().get(j).getName() + " FB address =>" + addressForFaceBook.get(j));
            }
        }
    }
}
